...
In the [previous post](https://shriv.github.io/e-flaneur/posts/bldg-heights-footprints/), we discussed the utility of retrieving a summarised height data from the overlaid raster cells of a particular building polygon. In this post, I look at a quick method do this using the package `exactextractr` [@exactextractr]. While typically used in the calculation of `zonal statistics`, it can also be applied to deriving features from raster layers for building polygons. 

Since I use `stars` to read and process raster data, I have an additional step before running the extraction function -  converting the `stars` object to a `raster` with a `st_as_raster()` convenience function [@262588213843476_convert].  
...
The `exact_extract()` function assigns mean, median, minimum etc. values from the raster layer to the polygons. The full set of summary operations can be seen [here](https://isciences.gitlab.io/exactextractr/#summary-operations). I've opted for `mean` and `median` as summary statistics. At first glance it appears that areas with tall buildings are being capture well e.g. around Liverpool Street station (e.g. the Shard) on the eastern part of City of London and the tall buildings in Canary Wharf. 
